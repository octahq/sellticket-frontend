...
export default function ConfirmCode({ textCenter }:Props) {
  ...
  const [count, setCount] = useState(59);
    ...
    const SuccessMessage = useMemo(() => (
      <p className="text-success-400">
        Your code is verified successfully
      </p>
    ), []);
    ...
    const messageToRender = isError ? ErrorMessage : isSuccess ? SuccessMessage : DefaultMessage;
  ...
  return (
    <div className={`${textCenter && 'text-center'}`}>
      <h2 className={`text-2xl text-[#071134] font-semibold mb-1 mt-3 `}>Confirm verification code</h2>
